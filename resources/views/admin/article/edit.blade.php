    ...
    <form action="/admin-article/{{ $article->id }}" method="post" enctype="multipart/form-data">
      ...
      @csrf
        <div class="form-control mx-4">
            ...
            <input type="text" id="title" name="title" placeholder="Title..." class="input input-primary input-bordered @error('title') input-error @enderror" value="{{ $article->title }}">
          </div> 
          @error('title')
          ...
          <div class="form-control mx-4">
            ...
            <input type="text" id="slug" name="slug" placeholder="Slug..."  value="{{ $article->slug }}" class="input input-primary input-bordered @error('slug') input-error @enderror">
            <i class="text-sm text-gray-400 mt-1">*if you click slug will be automatically filled if the input is clicked</i>
          </div> 
          @error('slug')
          <label class="label ml-4">
              <span class="label-text-alt text-red-500">{{ $message }}</span>
          ...
          <div class="form-control mx-4">
            ...
            <img src="{{url('articles/'.$article->thumbnail)}}" class="img-preview mb-4 rounded-lg">
            <img class="img-preview mb-4 rounded-lg">
                <div class="relative h-28 rounded-lg border-dashed border-2 @error('thumbnail') border-red-500 @enderror border-primary bg-white flex justify-center items-center hover:cursor-pointer">
                    ...
                    <!-- request untuk mengganti/menghapus gambar lama -->
                    <input type="hidden" name="deleteOldImage" value="{{ $article->thumbnail }}">
                    <!-- untuk input email baru -->
