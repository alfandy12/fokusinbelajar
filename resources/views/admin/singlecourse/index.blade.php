<x-app-layout>
    ...
    <div class="flex flex-wrap overflow-hidden mx-5">
        ...
        <div class="w-full overflow-hidden md:w-1/2 lg:w-1/2 xl:w-1/2 my-2">
          <!-- Column Content -->
          <a href="/admin-singlecourse/create" class="btn btn-primary ml-2">Add Data</a> 
        </div>
        <div class="w-full overflow-hidden md:w-1/2 lg:w-1/2 xl:w-1/2">
          <!-- Column Content -->
          <div class="form-control mx-2">
            <form action="/admin-singlecourse">
              @csrf
...
<div class="overflow-x-auto mx-5 rounded shadow-2xl">
    <table class="table table-zebra w-full">
      ...
      <tbody>
       @foreach ($singlecourse as $no => $single)
       <tr>
          <th>{{ $singlecourse->firstItem()+$no }}</th> 
          <td>{{ $single->category->name}}</td>
          <td>{{ Str::limit($single->title, 25)}}</td>
          <td>{{ $single->slug}}</td>
          <td><a href="https://www.youtube.com/channel/{{ $single->channel_id}}" target="_blank">{{ str::limit($single->channel_name, 10)}}</a></td>
          <td>{{ Str::limit($single->description, 35)}}</td>
          <td><div class="w-32">
             <img src="{{ $single->thumbnail}}" alt="" class="max-w-full rounded sm:rounded-lg"></td></div>
          <td>
            <a href="/admin-singlecourse/{{ $single->slug }}" class="btn btn-outline btn-sm btn-info mr-2"><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
            </svg></a>    
            <a href="/admin-singlecourse/{{ $single->slug }}/edit" class="btn btn-outline btn-sm btn-warning mr-2"><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
            </svg></a>
            <form action="/admin-singlecourse/{{ $single->slug }}" method="post" class="inline">
              @method('delete')
              @csrf
...
<div class="mx-5 my-5">{{$singlecourse->links()}}</div>
</x-app-layout>
